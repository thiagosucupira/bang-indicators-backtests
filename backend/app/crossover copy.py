    ...
    for i in range(1, len(df)):
        ...
        if position is None:
            ...
            if yesterday['EMA'] <= yesterday['SMA'] and today['EMA'] > today['SMA']:
                ...
                position = {
                    'Entry_Time': today.name,  # Using the index as the timestamp
        ...
        # Exit Conditions
            ...
            if position['Type'] == 'Long':
                # Exit Condition: EMA crosses below SMA
                if yesterday['EMA'] >= yesterday['SMA'] and today['EMA'] < today['SMA']:
                    exit_trade = True
            elif position['Type'] == 'Short':
                # Exit Condition: EMA crosses above SMA
                if yesterday['EMA'] <= yesterday['SMA'] and today['EMA'] > today['SMA']:
                    exit_trade = True
            
            if exit_trade:
                exit_price = today['Open']
                position['Exit_Time'] = today.name  # Using the index as the timestamp
    ...
    for _, trade in trades_df.iterrows():
        try:
            ...
            if trade_type == 'Long':
                ...
                # Long Exit: Downward-pointing triangle
                exit_color = 'blue' if trade_result == 'Win' else 'magenta'
                ax1.scatter(exit_time, exit_price, marker='v', color=exit_color, s=100, label='Long Exit' if not long_exit_plotted else "")
                ...
                ax1.scatter(exit_time, exit_price, marker='s', color=exit_color, s=100, label='Short Exit' if not short_exit_plotted else "")
